...
const renderer=new THREE.WebGLRenderer({antialias:true,alpha:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,1.7));renderer.setSize(innerWidth,innerHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.25;mount.appendChild(renderer.domElement);
...
function tick(t){const p=sy/innerHeight, approach=clamp(p/1.05,0,1), fade=clamp((p-.18)/.48,0,1), reveal=clamp((p-1.28)/.62,0,1);uniforms.uTime.value=t*.001;uniforms.uEnter.value=approach;
heroCopy.style.opacity=String(1-fade);heroCopy.style.transform=`translateY(${-fade*24}px)`;header.style.opacity=String(1-clamp((p-.7)/.45,0,.75));
const mobile=innerWidth<850, baseX=mobile?0:2.55;baseX;
world.position.x=mix(0,mobile?0:-.25,approach);world.position.z=mix(0,.65,approach);const s=mix(1,1.34,approach);world.scale.setScalar(s);sphere.position.x=baseX;glow.position.x=baseX;ring.position.x=baseX;ring2.position.x=baseX;
if(reveal>0){world.position.x=mix(world.position.x,-5.5,reveal);world.position.z=mix(.65,-3,reveal);world.scale.setScalar(mix(1.34,.72,reveal));world.rotation.z=mix(0,.12,reveal)}
